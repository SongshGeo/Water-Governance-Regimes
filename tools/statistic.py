...
def huanbi_rate(data, use_col, new_col):
    ...
    for i in range(0, len(data)):
        if i == 0:
            ...
            rate = format((data[use_col][i] - data[use_col][i - 1]) / data[use_col][i - 1], '.2%')
            result.append(rate)
    data[new_col] = result
...
def ratio_contribution(numerator, denominator, breakpoints=[1978, 1993]):
    ...
    for i in range(len(check_points)-1):
        ...
        # 分子分母的变化量
        numerator_change = np.log(numerator_sum.loc[end_year] / numerator_sum.loc[start_year])
        denominator_change = np.log(denominator.loc[start_year] / denominator.loc[end_year])
        
        # 分子分母各自的贡献率
        result[period]['Total'] = changes[period]
        result[period]['Numerator'] = changes[period] * numerator_change / ratio_change
        result[period]['Denominator'] = changes[period] * denominator_change / ratio_change
        
        # 各个分子的贡献率
        if len(numerator.shape) > 1:
            numerator_ratio = numerator.loc[start_year:end_year, :].sum() / numerator.loc[start_year:end_year, :].sum().sum()
            for col in numerator.columns:
                result[period][col] = numerator_ratio.loc[col] * result[period]['Numerator']
