...
def get_yr_gdp_data(how="gdp"):
    ...
    for col in gdp:
        ...
        if region:
            region_dict[region][col] = gdp[col]  # 如果是黄河流域的省份，存进对应的字典
        else:
            gdp.drop(col, axis=1, inplace=True)  # 不是黄河流域的省份，就丢掉
...
def plot_bar_by_category(
    ax, data, categories, colors, position, start_yr, end_yr, **kargs
):
...
def get_consumptions(region):
    ...
    while flag < index:
        consumption += (
            water_use.groupby(["Region", "Year"])
            .sum()
            .loc[REGIONS[flag]]["Total water use"]
        )
        flag += 1
...
def get_surface_groundwater_coefficient(region):
    ...
    WATERSHED_TO_SUBREGION = {"SR": SR, "UR": UR, "MR": MR, "DR": DR}
...
def plot_water(ax, start_yr, end_yr, colors):
    ...
    for i, region in enumerate(REGIONS):  # 循环每个区域
        ...
        bar = ax.bar(x=i, height=mean, yerr=std, color=colors[i])
        legends.append(bar)
    ax.set_xticks(np.arange(4))
    ax.set_xticklabels(REGIONS)
    ax.set_xlabel("Regions")
