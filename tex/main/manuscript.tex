...
\begin{abstract}
	...
	However, there is still lacking of effective method to detect regime shifts of water utilization linking to social changes, with much fewer attempts to develop theoretical models to explain their transition phases as well. 
	Here, by integrating three different dimensions of water utilization (stress, tendentiousness and pattern), we develop an Integrated Water Resources Utilization (IWRU) Index at a basin scale to detect regime shifts. 
	By applying this index to the Yellow River Basin, China, we summarized a general transition framework of regimes, which gives a sketch of relationships between human societies and their water utilization. 
	Based on the framework, transition phases within the three regimes and accompanying development traps can be well understood, as a useful guideline for the Yellow River to develop in a coordinated way.
\end{abstract}
