...
While the above three dimensions (stress, priority and configuration) also have own variation (see \textit{SI Appendix} Methods S4 and Fig. SX), combination of their contribution ratios clustered clearly by different time periods, indicating three regimes (Figure~\ref{fig:phases}).
\begin{figure}%[htbp]
	\centering
...
In short, agricultural expansion, industrial services expansion, water resources management, as well as scientific and technological progress and water use efficiency improvement, have driven the YRB to change regimes twice between 1965 and 2013, which had been along with social development in different aspects and can be interpreted as a transition process towards comprehensive water utilization.

\subsection*{Transition framework towards comprehensive water utilization}

\begin{figure*}[th!]
	\centering
	\includegraphics[width=\linewidth]{../../figures/main/transition.jpg}
	\caption{
		Transition framework towards a comprehensive water utilization, where water undertakes more socio-economic functions and needs to be considered holistically. Blue paths dominated by a natural water loop while red ones dominated by socio-economic loops. 
...
The association between the above transition process and the regime shifts of water utilization may be pervasive as socio-economic loops are gradually dominating human-water systems in most river basins.
% 因此，我们在总结黄河流域变化过程的基础上，进一步概念化了水资源利用制度的过渡框架。
As such, we summarized a transition framework of the water utilization regimes here, which conceptualizes a general trajectory towards a comprehensive water utilization (Figure~\ref{fig:summary}).
% 水资源利用制度包括压力、倾向性、格局三个重要因素，三个指标在过渡框架中随着不断变化。

Throughout the transition, three dimensions of water utilization are various in discipline of changes. 
% 水资源压力
...
%tag 研究方法
\matmethods{
Here, we constructed the Integrated Water Resources Utilization (IWRU) Index which consists of three dimensions and identified the regime in the changes of the index over time by change points detection. 
Each dimension is reflected by an indicator after normalization.
Finally, the contribution to changes of IWRU index along with each main indicators were calculated separately for each regime (i.e. period). 
	
	\subsection*{Integrated Water Resources Utilization (IWRU) Index}
	Assuming they have equal weights, the Integrated Water Resources Utilization (IWRU) Index consists of three dimensions (S: stresses, P: priority and C: configurations, denoted by sub-indicators correspondingly):

	$$ IWRU = I'_T + I'_P - I'_S $$

	% 其中 I 是代表每个维度的每个子指标
	Where $I'$ is normalized indicator of a certain dimension (P: Priority, C: Configuration, S: Stress). The direction of stress is different from the other two dimensions because the expected change of this indicator with social development is different (see \textit{SI Appendix} Methods S4).
	...
	To description of configurations $I_C$, we designed an indicator by imitation of information entropy, called Configuration Entropy Metric (CEL). CEL is a metric to measure the degree of evenness of water configuration (see \textit{SI Appendix} Methods S4), basin development is often accompanied by the reduction of regional differences and sectors' differences in the whole basin, but there is a complement of sectors between regions, i.e.:

	$$ I_C = \frac{CEL_{r} * CEL_{s}}{CEL_{rs}} $$
	
	where $CEL_{r}$ and $CEL_{s}$ are Configuration Entropy Metric in different regions and different sectors. $CEL_{rs}$ is differences between sectors in a certain region to the whole basin (see \textit{SI Appendix} Methods S4). 
