...
%DIF DEL diff/v-8.2.3.tex             Mon Oct  9 19:46:18 2023
%DIF ADD out/manuscript_v-8.3.2.tex   Fri Oct 13 10:05:48 2023
...
The Yellow River Basin (YRB), which contains the fifth-largest and most sediment-rich river in the world, needs integrated water governance because of geological and human history~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{mostern2021,best2019}}\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{mostern2021,best2019,wu2020a}}\hskip0pt%DIFAUXCMD
}\DIFaddend .
...
The SFV-index, which has many applications, is the most comprehensive index of water stress we know\DIFaddbegin \DIFadd{~\mbox{%DIFAUXCMD
\cite{qin2019}}\hskip0pt%DIFAUXCMD
}\DIFaddend .
	...
	First, for scarcity, $A_{i, j}$ is the total water \DIFdelbegin \DIFdel{consumption }\DIFdelend \DIFaddbegin \DIFadd{use }\DIFaddend as a proportion of regional multi-year average runoff volume in year $j$ and region $i$ (in this study, four regions in the YRB, \textit{Supporting Information Section S1}):
	\begin{equation}
...
\DIFaddbegin \DIFadd{For robustness, we tried different change points detection methodologies~\mbox{%DIFAUXCMD
\cite{matteson2014,killick2012,bai1997,keogh2001} }\hskip0pt%DIFAUXCMD
and the results are close (see }\textit{\DIFadd{Supporting Information~S4}} \DIFadd{and Table~S2).
		}\DIFaddend We used $\alpha = 0.001$ as the threshold level of the p-value, meaning that the probability of a statistically significant change-point judgment being valid was more than $99.9\%$. We divided the series into two at that point and analyzed each series separately until all significant change points were detected. Though two break points in the main text with $\alpha = 0.001$, the threshold from $0.0005$ to $0.05$ does not affect our results, and the \DIFdelbegin \DIFdel{breakpoints }\DIFdelend \DIFaddbegin \DIFadd{change points }\DIFaddend we identified are robust (see Figure~S6).
...
Two significant \DIFdelbegin \DIFdel{breakpoints }\DIFdelend \DIFaddbegin \DIFadd{change points }\DIFaddend divide the changes in the IWGI into three periods, with different contributions from three aspects (Figure~\ref{fig:IWGI}A).
...
\begin{figure*}[th!]
	...
	\caption{
		...
		\textbf{D.} Social atmosphere\DIFaddbeginFL \DIFaddFL{$^*$ }\DIFaddendFL (red triangles) and national-level governance policies (the circles, different colours denote signed by different state agencies). The light grey bars count official documents related to the YRB on a basinal scale (the Yellow River Events).
		\DIFaddbeginFL \footnotesize{$^*$ Here, ``social atmosphere'' refers to the sociocultural context in which people live or in which something happens, including the culture that the individual was educated or lives.}
	\DIFaddendFL }\label{fig:Causes}
\end{figure*}
...
Environmental \DIFdelbegin \DIFdel{context, social atmosphere, and }\DIFdelend \DIFaddbegin \DIFadd{and social contexts, governance }\DIFaddend policies played roles in all three periods.
...
Ensuing the P2, the number of new reservoirs decreased but boosted their role in water regulating and management (more red circles in Figure~\ref{fig:Causes}~C, and larger water conveyance variability in Supporting Informations Figure~\DIFdelbegin \DIFdel{S7}\DIFdelend \DIFaddbegin \DIFadd{S8}\DIFaddend ).
...
In the case of the YRB, our results show that there have been three distinct governance regimes; we named them: a massive supply regime (P1: $1965 \sim 1978$), a governance transforming regime (P2: $1979 \sim 2001$), and an adaptation oriented regime (P3: $2002 \sim 2013$) (Figure~\ref{fig:IWGI} \DIFdelbegin \DIFdel{).
}\DIFdelend \DIFaddbegin \DIFadd{and Table~}\ref{tab:regimes}\DIFadd{).
These regime shifts, as comprehensive outcomes of complex human-water interactions, are coincidentally well presented by several well-known and dominated social atmospheres in China (Table~}\ref{tab:regimes}\DIFadd{).
...
\DIFaddend During the massive supply regime \DIFdelbegin \DIFdel{with lower water stress }\DIFdelend ($1965 \sim 1978$ in the YRB), water governance \DIFdelbegin \DIFdel{thus }\DIFdelend tended to boost water supply for services (mainly provisioning purposes then -livestock and crops) by constructing reservoirs and channels (Figure~\ref{fig:Causes}~B).
...
\DIFaddbegin \begin{table}[htbp]
    \centering
    \caption{\DIFaddFL{Brief summary of regime transitions for water governance at the Yellow River Basin}}
      \begin{tabularx}{\textwidth}{p{0.15\textwidth} p{0.15\textwidth} p{0.2\textwidth} p{0.5\textwidth}}
      \toprule
      \DIFaddFL{Time  }& \DIFaddFL{Closet social atmosphere$^*$ }& \DIFaddFL{Regime }& \DIFaddFL{Main trait }\\
      \midrule
      \DIFaddFL{$1965-1978$ }& \DIFaddFL{1960s: Conquering nature }& \DIFaddFL{massive supply regime }& \DIFaddFL{boost water supply for services (mainly provisioning purposes then -livestock and crops) by constructing reservoirs and channels }\\
      \DIFaddFL{$1979-2001$ }& \DIFaddFL{1978: reform and opening-up }& \DIFaddFL{governance transforming regime }& \DIFaddFL{triggered institutional changes for rapid enhancement of overall social adaptive capacity to severe water stresses }\\
      \DIFaddFL{$2002-2013$ }& \DIFaddFL{2002: Environmental regulating }& \DIFaddFL{adaptation-oriented regime }& \DIFaddFL{adapting to stable high water stress with trade-offs between water-dependent regions and sectors }\\
      \bottomrule
      \end{tabularx}\label{tab:regimes}

      \footnotesize
      \DIFaddFL{$^*$ Here, social atmosphere refers to the sociocultural context in which people live or in which something happens, including the culture that the individual was educated or lives.
  }\end{table}
\DIFaddend The start of the governance transforming regime (P2: $1979 \sim 2001$) coincided with rising competition for water use after the ``reform and opening-up'' (Figure~\ref{fig:Causes}~C).
...
To support water use in early stage (Figure~\ref{fig:summary}~A), strategies tend to manage natural water \DIFdelbegin \DIFdel{processes }\DIFdelend \DIFaddbegin \DIFadd{cycle }\DIFaddend in order to maintain the provisioning (larger) and non-provisioning water (less).
...
\DIFdelbegin \DIFdel{In an analogy at a global scale, the resource challenges, represented by water shortage and water supplying difficulties, are mainly faced by undeveloped and developing basins~\mbox{%DIFAUXCMD
\cite{allan2019,speed2013,liu2012}}\hskip0pt%DIFAUXCMD
.
Highly-controlled }\DIFdelend \DIFaddbegin \DIFadd{The Yellow River basin's challenges as an example, represented by the shift in national narratives from conquering nature for economy and eliminating pollution to stressing of harmonious human-water, increased the importance of administrative measures in resolving water-related disputes.
It demonstrates again that highly-controlled }\DIFaddend and developed basins (especially for transboundary rivers) must \DIFdelbegin \DIFdel{mainly }\DIFdelend resolve structural challenges, such as water disputes or lack of equity, and may be in urgent need of novel flexible, efficient sociopolitical governance structures~\cite{unep-dhi2016,mirumachi2015}.
