...
%DIF DEL diff/v-8.0.0.tex   Tue May  9 11:13:06 2023
%DIF ADD diff/draft.tex     Tue May  9 17:21:31 2023
...
Water governance refers to the political, social, economic, and administrative systems that influence the use and management of water\DIFaddbegin \DIFadd{~}\DIFaddend \cite{oecd2018, wang2017}, essentially about ``who gets water, when and how''\cite{lasswell2018}.
Therefore, the United Nations Development Programme (UNDP) has suggested that three core aspects of water use decided by water governance correspondingly: ``When and what water to use?'' (stress), ``How does water provide different services for human well-being?'' (purpose), and ``Who can use water equally and efficiently?'' (allocation)\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{undpwatergovernancefacility2016}}\hskip0pt%DIFAUXCMD
.
First, water stress depends not only on climate (with increasing scarcity and uncertainty in many regions)but also on the }\DIFdelend \DIFaddbegin \DIFadd{~\mbox{%DIFAUXCMD
\cite{undpwatergovernancefacility2016}}\hskip0pt%DIFAUXCMD
.
...
However, a comprehensive and detailed list of the key components for assessment of water governance is rare, hence quantitative studies usually propose indexes by using proxy dataset of human activities as a simpler way~\mbox{%DIFAUXCMD
\cite{varis2019}}\hskip0pt%DIFAUXCMD
.
A qualitative approach to the assessment of water governance can list more elements regarding to water governance, where OECD's index and assessment dataset are the most widely used. From the water side, intuitive indexes can provide an embodiment of governance outcomes.
...
Second, the purpose of how water services human well-being is \DIFdelbegin \DIFdel{to consider }\DIFdelend \DIFaddbegin \DIFadd{tilting the }\DIFaddend trade-offs between \DIFdelbegin \DIFdel{consumptive }\DIFdelend \DIFaddbegin \DIFadd{provisioning }\DIFaddend uses (e.g., drinking and food production) and \DIFdelbegin \DIFdel{non-consumptive }\DIFdelend \DIFaddbegin \DIFadd{non-provisioning }\DIFaddend uses (e.g., energy production)\DIFaddbegin \DIFadd{~}\DIFaddend \cite{liu2017,florke2018,jaeger2019}.
Third, the allocation of water across the whole basin is not only decided by regionally socio-economic and environmental context but also influenced by \DIFdelbegin \DIFdel{systematic regulation
}\DIFdelend \DIFaddbegin \DIFadd{growing systematic regulations~}\DIFaddend \cite{schmandt2021,speed2013}.
...
A first critical step in understanding the successes and failures of water governance is to identify the different regimes that underpin it\DIFaddbegin \DIFadd{~}\DIFaddend \cite{kjellen2015, grafton2013}.
Regimes of water governance arise within linked human-water systems (based on management, institutions, and exploitation) to create local equilibria in social-ecological structures and functions\DIFaddbegin \DIFadd{~}\DIFaddend \cite{falkenmark2021,bressers2013,loch2020}.
