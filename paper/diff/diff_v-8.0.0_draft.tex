...
%DIF ADD diff/draft.tex     Fri May 12 23:08:33 2023
...
\title{Identifying regime transitions for water governance\DIFdelbegin \DIFdel{at a basin scale}\DIFdelend \DIFaddbegin \DIFadd{: Take the Yellow River Basin in China as case}\DIFaddend }
...
For quantitative studies, due to the lack of comprehensive and detailed information on key components for water governance assessment, these studies often resort to proxy datasets of human activities to create simpler indices~}\cite{varis2019,huggins2020}\DIFadd{.
...
The SFV-index, which has many applications, is the most comprehensive index of water stress we know.
	...
	\DIFaddend Based on the hydrological and economic context of YRB, four second-level regions are divided (Source Region, Upper Region, Middle Region, and Lower Region, see \textit{Supporting Information S1}).
...
\DIFdelend \DIFaddbegin \DIFadd{$RC_i$ is the total storage capacities of reservoirs in the region $i$, $R_{i, std}$ is the standard deviation of runoff in the region $i$}\DIFaddend .
	...
	%DIFAUXCMD
\DIFdelend \DIFaddbegin \DIFadd{First, for scarcity, $A_{i, j}$ is the total water consumption as a proportion of regional multi-year average runoff volume in year $j$ and region $i$ (in this study, four regions in the YRB, }\\\DIFadd{textit}{\DIFadd{Supporting Information}}\DIFadd{~\ref{secA1}):
	}\begin{equation}
...
We used $\alpha = 0.001$ as the threshold level of the p-value, meaning that the probability of a statistically significant change-point judgment being valid was more than $99.9\%$. We divided the series into two at that point and analyzed each series separately until all significant change points were detected. Though two break points in the main text with $\alpha = 0.001$, the threshold from $0.0005$ to $0.05$ does not affect our results, and the breakpoints we identified are robust (see \\textit{Supporting Information} Figure~S3).
	...
	\subsection{Datasets}
\DIFdelbegin \DIFdel{In order to calculate IWGI in the YRB, all the datasets we need are listed in the }\textit{\DIFdel{SI Appendix}} %DIFAUXCMD
\DIFdel{Table~S1 with a detailed description in }\textit{\DIFdel{Supporting Information S3}}%DIFAUXCMD
\DIFdelend \DIFaddbegin \DIFadd{Multiple sources of data were used in this study, and we have attached a summarized data table as supplementary material and their processing are detailed described in the supplementary documentation}\DIFaddend .
...
We developed an Integrated Water Governance Index (IWGI) to detect regime shifts in water governance by integrating them. Applying the IWGI to a rapidly-changing large river basin (the Yellow River Basin, China), we interpret how water governance shifts between three regimes over half a century\DIFaddbegin \DIFadd{.
...
Last, adaptation-oriented regime (P3: $2002 \sim 2013$) with stable high water stress resulted in trade-offs and joint regulating between water-dependent regions and sectors}\DIFaddend .
